#!/usr/bin/env python3
"""
Single Client Test Data Generator

Generates realistic lead sessions for a specific client.
Used by individual client generation scripts.
"""
...
def generate_client_leads(client_name: str, num_leads: int = 30) -> str:
    ...
    # Add header comment
    business_names = {
        'pawsome': 'Pawsome Dog Walking (Pet Services)',
        'metro_realty': 'Metro Realty Group (Real Estate)',
        'techsolve': 'TechSolve Consulting (Software Consulting)',
        'fitlife': 'FitLife Personal Training (Health & Fitness)',
        'sparkle_clean': 'Sparkle Clean Solutions (Home Cleaning)'
    }
    
    sql_statements.append(f"""-- Test Data for {business_names.get(client_name, client_name.upper())}
-- Generated {num_leads} realistic lead sessions with complete tracking and outcomes
-- Client: {client_name} | Form: {form_id} | Generated: {datetime.now().isoformat()[:19]}
...
""")
    ...
    sql_statements.append(f"""
SELECT 'Generated {num_leads} realistic lead sessions for {client_name}!' as status;
""")
