...
CREATE POLICY "Service role full access to form_questions" ON form_questions
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to lead_sessions" ON lead_sessions
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to responses" ON responses
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to tracking_data" ON tracking_data
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to session_snapshots" ON session_snapshots
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Service role full access to lead_outcomes" ON lead_outcomes
    FOR ALL USING (auth.role() = 'service_role');
...
CREATE POLICY "Anonymous can view active forms" ON forms
    FOR SELECT USING (
        auth.role() = 'anon' AND is_active = true
    );

-- Anonymous users can view questions for active forms
CREATE POLICY "Anonymous can view questions for active forms" ON form_questions
    FOR SELECT USING (
        auth.role() = 'anon' AND
        EXISTS (
            SELECT 1 FROM forms 
...
CREATE POLICY "Anonymous can create sessions" ON lead_sessions
    FOR INSERT WITH CHECK (
        auth.role() = 'anon' AND
        EXISTS (
            SELECT 1 FROM forms 
            WHERE forms.id = lead_sessions.form_id 
            AND forms.is_active = true
        )
    );

-- Anonymous users can update their own sessions (for session state)
CREATE POLICY "Anonymous can update sessions" ON lead_sessions
    FOR UPDATE USING (auth.role() = 'anon');

-- Anonymous users can select their sessions (for session continuation)
CREATE POLICY "Anonymous can select sessions" ON lead_sessions
...
CREATE POLICY "Anonymous can insert tracking_data" ON tracking_data
    FOR INSERT WITH CHECK (auth.role() = 'anon');

-- Anonymous users can insert/update session snapshots
CREATE POLICY "Anonymous can insert session_snapshots" ON session_snapshots
    FOR INSERT WITH CHECK (auth.role() = 'anon');

CREATE POLICY "Anonymous can update session_snapshots" ON session_snapshots
    FOR UPDATE USING (auth.role() = 'anon');
...
-- Migration complete
SELECT 'Simplified Row Level Security implemented successfully!' as status;